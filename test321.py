...
w.title('Python - Calculadora - JOHN')
...
def botao_mult():
    ...
    if em_operacao == 0:
        ...
        if acumulado != 0:
            ...
            if (texto[len(texto) - 1]) == '0' and (texto[len(texto) - 2]) == '.':
...
def botao_equ():
    ...
    if operacao_anterior != 'igual':
        ...
        if (texto[len(texto) - 1]) == '0' and (texto[len(texto) - 2]) == '.' or str(tela['text']) == '0':
...
w.mainloop()'''
names_TXT =('C:\\Users\\Jk_cu\\OneDrive\\Documentos\\NomesFaces.txt')

def AddName():   # adicionar nomes + ID
    Name = input('Digite seu nome:')
    Info = open(names_TXT, "r+")
